class slave_monitor extends uvm_monitor;
	...
	slave_config m_cfg;
	virtual axi_intf.SLAVE_MON_MP vif;
		...
		for(int i=0;i<=xtn.AWLEN;i++)
			begin
				...
				if(vif.slave_mon_cb.WSTRB == 15)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[31:0];
				
				if(vif.slave_mon_cb.WSTRB == 14)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[31:8];

				if(vif.slave_mon_cb.WSTRB == 7)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[23:0];
		
				if(vif.slave_mon_cb.WSTRB == 12)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[31:16];
	
				if(vif.slave_mon_cb.WSTRB == 3)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[15:0];

				if(vif.slave_mon_cb.WSTRB == 8)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[31:24];

				if(vif.slave_mon_cb.WSTRB == 4)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[23:16];
	
				if(vif.slave_mon_cb.WSTRB == 2)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[15:8];

				if(vif.slave_mon_cb.WSTRB == 1)
					xtn.WDATA[i] = vif.slave_mon_cb.WDATA[7:0];

				xtn.WSTRB[i] = vif.slave_mon_cb.WSTRB;

				xtn.WLAST = vif.slave_mon_cb.WLAST;
				xtn.WVALID = vif.slave_mon_cb.WVALID;
				xtn.WREADY = vif.slave_mon_cb.WREADY;
	...
	task collect_bresp(axi_xtn xtn);
		...
		xtn.BRESP = vif.slave_mon_cb.BRESP;
		
		xtn.BVALID = vif.slave_mon_cb.BVALID;
		xtn.BREADY = vif.slave_mon_cb.BREADY;
	...
	task collect_raddr(axi_xtn xtn);
		...
		xtn.ARREADY = vif.slave_mon_cb.ARREADY;
